...
Only the existing native desktop supervisor, new Pi web launcher/helper and
userdata enable marker are changed. Exact previous hashes prevent clobbering
an independently edited deployment. Default is a host-only manifest.
"""
import argparse
import base64
import hashlib
...
SPECS={
    '/usr/local/bin/start-persistent-desktop': (
        ...
        '081d97e3afcb80da84b3d27ff0cc7901be8d53bc980af02345c6b19ff0b9d851'),
    '/srv/s22/agent-web/start-agent-web.py': (
        Path(__file__).with_name('start-agent-web.py'),
        '649b9f93bfe23f6f01e48ab26e303c4c8a5cc7c2eebdc40fdb96f0d78a9887c6'),
    '/srv/s22/arch/usr/local/bin/pi-web-session': (
        Path(__file__).with_name('pi-web-session'),
        'c58e7ce052e73fe13e09e5e2c1a6f00b06a244279af84693fdab44e973d86751'),
}
...
assert set(incoming)=={'/usr/local/bin/start-persistent-desktop','/srv/s22/agent-web/start-agent-web.py','/srv/s22/arch/usr/local/bin/pi-web-session'}
...
for name,item in incoming.items():
 target=p(name); st=target.stat()
 assert not target.is_symlink() and st.st_uid==0 and not st.st_mode & 0o022
 assert hashlib.sha256(target.read_bytes()).hexdigest()==item['before'], name
...
receipt={}
for name,item in incoming.items():
 target=p(name); (backup/target.name).write_bytes(target.read_bytes())
...
def main():
    ...
    payload={name:{'before':old,'after':hashlib.sha256(local.read_bytes()).hexdigest(),
             ...
             for name,(local,old) in SPECS.items()}
