...
def main():
    ...
    if root.parent != Path('/srv/s22/gpu-compat-20260921') or root.name not in {
        'base', 'opencl-real', 'opencl-headless', 'vulkan-headless', 'llama-vulkan'}:
        raise SystemExit('Unexpected isolated runtime root')
    argv = sys.argv[2:]
    mounts = []
    bridge_version = None
    while argv and argv[0] in {'--proc', '--sys', '--bridge-v2', '--bridge-v3', '--bridge-v4'}:
        option=argv.pop(0)
        if option.startswith('--bridge-'):
            assert root.name == 'llama-vulkan'
            assert bridge_version is None
            bridge_version=option.removeprefix('--bridge-')
            assert (root/('vendor/lib64/bridge-'+bridge_version+'/libvulkan.so')).is_file()
        else:
            mounts.append(option[2:])
    ...
    if bridge_version:
        env['LD_LIBRARY_PATH']='/vendor/lib64/bridge-'+bridge_version+':'+env['LD_LIBRARY_PATH']
