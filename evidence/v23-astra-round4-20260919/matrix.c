...
static int hook_open(const char*p,int f,...){
 ...
 if(mode==57&&!strcmp(p,"/cache/v23_log.txt")){mode=0;errno=EACCES;return -1;}
 int fd=open(p,f,m);
 if(!strcmp(p,"/cache/v23_boot_count.txt.tmp"))tempfd=fd;
...
static ssize_t hook_write(int fd,const void*b,size_t n){
 if(mode==50&&fd==tempfd){errno=ENOSPC;return -1;}
 if(mode==53&&fd==tempfd){if(!writes++){errno=EINTR;return -1;}if(n>3)n=3;}
 if(mode==32&&fd==flushfd){mode=0;errno=ENOSPC;return -1;}
 return write(fd,b,n);
}
static int hook_close(int fd){int r=close(fd);if(fd==tempfd)tempfd=-1;if(fd==cachefd)cachefd=-1;if(mode==33&&logopens==1){mode=0;raise(SIGABRT);}return r;}
static pid_t hook_fork(void){pid_t p=fork();if(!p&&mode==21){mode=0;raise(SIGSEGV);}return p;}
...
static int hook_execl(const char*p,const char*a,...){(void)p;(void)a;if(mode==20)raise(SIGSEGV);errno=ENOENT;return -1;}

static int hook_fsync(int fd){
 if((mode==51&&fd==tempfd)||(fd==cachefd&&((mode==54&&logopens==1)||(mode==55&&logopens==2)||mode==56))){errno=EIO;return -1;}return fsync(fd);
}
static int hook_rename(const char*a,const char*b){if(mode==52){errno=EIO;return -1;}return rename(a,b);}
#define fsync hook_fsync
...
static void reset(void){mode=logopens=reads=writes=0;tempfd=cachefd=-1;cmdfd=flushfd=-1;g_cache_ok=0;early_log_n=0;unlink("/cache/v23_boot_count.txt");unlink("/cache/v23_boot_count.txt.tmp");unlink("/cache/v23_log.txt");if(ftruncate(g_kmsg_fd,0)||lseek(g_kmsg_fd,0,SEEK_SET)<0)exit(2);}
...
int main(int argc,char**argv){
 ...
 for(int k=50;k<=57;k++){
  reset();put("/cache/v23_boot_count.txt","123456");log_line("EARLY\n");mode=k;
  int bn=cache_sequence();mode=0;printf("MATRIX mode=%d boot=%d cache_ok=%d queued=%d writes=%d\n",k,bn,g_cache_ok,early_log_n,writes);show("/cache/v23_boot_count.txt");show("/cache/v23_log.txt");
 }
 reset();int before=0,after=0;for(int f=0;f<256;f++)if(fcntl(f,F_GETFD)>=0)before++;
 for(int i=0;i<100;i++){reset();cache_sequence();}
 for(int f=0;f<256;f++)if(fcntl(f,F_GETFD)>=0)after++;
 printf("FD LEAK 100 counter/banner/flush sequences before=%d after=%d\n",before,after);
 return 0;
}
