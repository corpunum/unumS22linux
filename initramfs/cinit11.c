...
static void log_line(const char *msg) {
	...
	if (g_cache_ok) {
		int fd = open("/cache/v22_log.txt", O_WRONLY | O_CREAT | O_APPEND, 0644);
		if (fd >= 0) {
...
static void logf_uptime(const char *prefix) {
	...
	snprintf(buf, sizeof(buf), "[v22 t=%d.%03ds] %s\n", sec, frac, prefix);
	log_line(buf);
}
...
int main(void) {
	...
	{
		char m[220];
		snprintf(m, sizeof(m), "early syscalls: dev=%d devtmpfs=%d devblock=%d proc=%d sys=%d "
		         "kmsg_n=%d null_n=%d con_n=%d wd_n=%d sda33_n=%d procmnt=%d sysmnt=%d (0=ok, else errno)",
	...
	if (g_wd_fd >= 0) {
		...
		logf_uptime("NOTE: tmr_atboot=1 arms the watchdog at probe (first build in the series to do so); "
		            "running on the raw DT default timeout above, NOT stock watchdogd's 30s WDIOC_SETTIMEOUT window");
	} else {
	...
	if (wait_for_partition("sda33", 15)) {
		...
		if (mount("/dev/block/sda33", "/cache", "ext4", MS_SYNCHRONOUS, NULL) == 0) {
			...
			int boot_n = -1; /* -1 = counter unavailable, logged as such */
			{
				...
				int cfd = open("/cache/v22_boot_count.txt", O_RDONLY);
				if (cfd < 0 && errno == ENOENT) {
					...
					close(cfd);
					if (total > 0 && total < sizeof(cb) - 1) {
						size_t dl = 0;
						while (dl < total && cb[dl] >= '0' && cb[dl] <= '9') dl++;
						int trailing_ok = (dl == total) || (dl == total - 1 && cb[dl] == '\n');
						/* cap at 7 digits (<=9,999,999) - nowhere near a real
						 * crash-loop count, comfortably below INT_MAX/2, so
						 * "+1" can never overflow regardless of digit count
						 * accepted here. */
				...
				if (prev >= 0) {
					...
					int tfd = open("/cache/v22_boot_count.txt.tmp", O_WRONLY | O_CREAT | O_TRUNC, 0644);
					if (tfd >= 0) {
						char wb[16];
						int wl = snprintf(wb, sizeof(wb), "%d", candidate);
						if (write_full(tfd, wb, (size_t)wl) == 0 && fsync(tfd) == 0) {
							close(tfd);
							if (rename("/cache/v22_boot_count.txt.tmp", "/cache/v22_boot_count.txt") == 0) {
								boot_n = candidate;
							}
						} else {
							close(tfd);
			...
			char banner[96];
			if (boot_n >= 0) {
				snprintf(banner, sizeof(banner), "[v22] ===== BOOT ATTEMPT #%d (V22/cinit11) =====\n", boot_n);
			} else {
				snprintf(banner, sizeof(banner), "[v22] ===== BOOT ATTEMPT #unknown, counter unavailable/untrustworthy (V22/cinit11) =====\n");
			}
	...
	if (g_cache_mounted) {
		...
		if (un == 0) {
			...
			char kb[100];
			int kn = snprintf(kb, sizeof(kb), "[v22] umount(/cache) FAILED errno=%d, cache remains mounted\n", errno);
	...
	for (;;) {
		...
		if (g_kmsg_fd >= 0) {
			char kb[80];
			int kn = snprintf(kb, sizeof(kb), "[v22] reboot() FAILED errno=%d attempt=%d, retrying\n", errno, attempt);
			ssize_t w = write(g_kmsg_fd, kb, kn);
			(void)w;
		}
