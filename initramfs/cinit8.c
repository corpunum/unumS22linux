...
/* Check the actual bound DEVICE INSTANCE symlink, not just driver directory
 * existence (which only proves the driver registered, not that THIS device
 * probed). Driver names verified live against the real phone - they do NOT
 * match the .ko module names (e.g. module dwc3-exynos-usb registers driver
 * "exynos-dwc3", not "dwc3-exynos"). exynos-ufs has suppress_bind_attrs, so
 * the device symlink is the only available proof for it. */
static int platform_bound(const char *drv, const char *dev) {
	char path[160];
	snprintf(path, sizeof(path), "/sys/bus/platform/drivers/%s/%s", drv, dev);
...
static void modprobe(const char *name) {
	...
	if (pid < 0) {
		...
		snprintf(msg, sizeof(msg), "modprobe %s: fork() FAILED errno=%d\n", name, errno);
		logf_uptime(msg);
		return;
	}
	if (pid == 0) {
...
int main(void) {
	 ...
	 * driver/device-instance name pairs against the real phone. */
	...
	for (int i = 0; i < 4; i++) {
		char m[100];
		snprintf(m, sizeof(m), "platform driver %s/%s bound=%d\n",
	...
	{
		char m[100];
		snprintf(m, sizeof(m), "reached absolute t=45s alive, last successful pet at t=%.3fs - self-rebooting now (PSCI restart)\n", g_last_pet_ok);
		logf_uptime(m);
	}
	sync();
